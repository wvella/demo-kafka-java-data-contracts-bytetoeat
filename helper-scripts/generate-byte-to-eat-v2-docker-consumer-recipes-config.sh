...
if [[ "$CLOUD" == "AZURE" ]]; then
  ...
  CLIENT_ID=$(terraform -chdir="$TERRAFORM_DIR" output -raw azure_java_consumer_client_id)
  CLIENT_SECRET=$(terraform -chdir="$TERRAFORM_DIR" output -raw azure_java_consumer_client_secret)
  ...
  CLIENT_ID=$(terraform -chdir="$TERRAFORM_DIR" output -raw aws_java_consumer_client_id)
  CLIENT_SECRET=$(terraform -chdir="$TERRAFORM_DIR" output -raw aws_java_consumer_client_secret)
  ...
  CLIENT_EMAIL=$(terraform -chdir="$TERRAFORM_DIR" output -raw gcp_java_consumer_client_email)
  CLIENT_SECRET_ID=$(terraform -chdir="$TERRAFORM_DIR" output -raw gcp_java_consumer_client_secret_id)
  CLIENT_ID=$(terraform -chdir="$TERRAFORM_DIR" output -raw gcp_java_consumer_client_id)
  CLIENT_SECRET_ENCODED=$(terraform -chdir="$TERRAFORM_DIR" output -raw gcp_java_consumer_client_secret)
