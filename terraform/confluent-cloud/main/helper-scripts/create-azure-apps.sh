...
echo "tenant-id = \"$TENANT_ID\"" >> "$TFVARS_FILE"
echo "subscription-id = \"$SUBSCRIPTION_ID\"" >> "$TFVARS_FILE"
...
echo "resource-group = \"$RESOURCE_GROUP\"" >> "$TFVARS_FILE"
...
create_app() {
    ...
    TFVARS_KEY_PREFIX=$(echo "$APP_NAME" | sed "s/-$UNIQUE_ID//g")
    echo "${TFVARS_KEY_PREFIX}-client-id = \"$APP_ID\"" >> "$TFVARS_FILE"
    if [[ -n "$CLIENT_SECRET" ]]; then
        echo "${TFVARS_KEY_PREFIX}-client-secret = \"$CLIENT_SECRET\"" >> "$TFVARS_SECRET_FILE"
    fi
}
